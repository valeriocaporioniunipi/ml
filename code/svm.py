...
from utils import scorer, get_data, mean_euclidean_error
...
def predict(model, x_ts, x_its, y_its):
    ...
    iloss = tf.eval(mean_euclidean_error(y_its, model.predict(x_its)))
...
def sklearn_svm(ms=False):
    ...
    tr_losses = mean_euclidean_error(y_tr, model.predict(x_tr))
    val_losses = mean_euclidean_error(y_vl, model.predict(x_vl))
